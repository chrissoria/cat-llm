...
consistent and reproducible results."""
    ...
    if categories:
        ...
        story.append(Paragraph(
            "For complete results including category counts, agreement scores, and visualizations, "
            "see the Classification Summary and Category Distribution sections on the following pages.",
            normal_style))
        story.append(Spacer(1, 15))
...
with col_input:
    ...
    if input_type_choice == "Survey Responses":
        ...
        st.info(
            "**Scope:** Each image is classified as a whole — suited for categorizing a *set* of images "
            "(e.g., labeling survey photos, social media posts, or field images by type). "
            "It is not designed for detecting or locating elements *within* a single image (e.g., object detection, segmentation).\n\n"
            "**Common failure modes:** very low-resolution or heavily compressed images; images where the "
            "relevant subject is small, cropped, or peripheral; images with ambiguous content that doesn't "
            "map clearly to any category. If results seem off, try sharpening your category descriptions or "
            "adding an 'Unclear / does not apply' catch-all category."
        )
