...
# Define the models, sources, and API keys
models = ["gpt-4o-2024-11-20", "claude-3-7-sonnet-20250219", "llama-3.1-sonar-large-128k-online", "mistral-large-latest"]
model_source = ["OpenAI", "Anthropic", "Perplexity", "Mistral"]
api_keys = ["OPENAI_API_KEY", "ANTHROPIC_API_KEY", "PERPLEXITY_API_KEY", "MISTRAL_API_KEY"]

# Extract multi-class categories using the defined models using catllm
for model, source, key in zip(models, model_source, api_keys):
    move_reason = cat.extract_multi_class(
...
for model, source, key in zip(models, model_source, api_keys):
    new_friends = cat.extract_multi_class(
...
for model, source, key in zip(models, model_source, api_keys):
    family_support = cat.extract_multi_class(
