...
- **Over-classification** is the dominant error here: with several categories per response, models tend to assign labels too liberally, especially on ambiguous text. Recall looks fine; precision does not.
...
- **Aim:** put the whole loop inside Stata, and correct over-classification from several evidence-based angles: category definitions, model choice, and cross-provider unanimous consensus.
:::
...
<table>
  ...
  <tbody>
    ...
    <tr class="highlight"><td>Multi-vendor unanimous ensemble</td><td>Sum of parts</td><td>Slowest member</td><td>Conservative</td><td>Yes</td></tr>
...
::: {.fig-note}
Single models of every size tend to assign categories too liberally. A unanimous ensemble of three diverse models corrects for it: pick it when a false positive costs more than a missed code, and when the categories are interpretive, where the precision gain is largest. For restricted-use data under an IRB or data-use agreement, the local route is often the only admissible one — and it is the route the package defaults to protecting, auto-starting the local server and enabling JSON recovery for smaller models.
:::
:::
:::
...
<div class="stat-strip">
  <div class="stat-cell"><span class="big">+38%</span><span class="cap">labels vs. human coders,<br>best single model</span></div>
...
- [0.85 vs. 0.78]{.stat-number} [**Run an ensemble with one option.** `consensus("unanimous")` over three diverse models lifts precision from 0.78 to 0.85 and beats the best single model on F1, correcting for over-classification with no hand-merging]{.stat-label}
