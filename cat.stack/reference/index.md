# Package index

## Classify & extract

Assign categories or discover categories from text, images, or PDFs.

- [`classify()`](https://christophersoria.com/cat-llm/cat.stack/reference/classify.md)
  : Classify text, images, or PDFs using LLMs
- [`extract()`](https://christophersoria.com/cat-llm/cat.stack/reference/extract.md)
  : Extract categories from text, images, or PDFs using LLMs
- [`explore()`](https://christophersoria.com/cat-llm/cat.stack/reference/explore.md)
  : Explore raw categories in text data

## Summarize

- [`summarize()`](https://christophersoria.com/cat-llm/cat.stack/reference/summarize.md)
  : Summarize text, images, or PDFs using LLMs

## Prompt optimization

Interactive, human-in-the-loop tuning of the system prompt.

- [`prompt_tune()`](https://christophersoria.com/cat-llm/cat.stack/reference/prompt_tune.md)
  : Optimize a classification prompt with human-in-the-loop feedback

## Installation

- [`install_cat_stack()`](https://christophersoria.com/cat-llm/cat.stack/reference/install_cat_stack.md)
  : Install the cat-stack Python package

## Local Ollama

- [`ensure_ollama_running()`](https://christophersoria.com/cat-llm/cat.stack/reference/ensure_ollama_running.md)
  : Ensure a local Ollama server is running
- [`check_ollama_model()`](https://christophersoria.com/cat-llm/cat.stack/reference/check_ollama_model.md)
  : Check whether a specific Ollama model is installed locally
- [`list_ollama_models()`](https://christophersoria.com/cat-llm/cat.stack/reference/list_ollama_models.md)
  : List locally installed Ollama models
- [`pull_ollama_model()`](https://christophersoria.com/cat-llm/cat.stack/reference/pull_ollama_model.md)
  : Pull (download) an Ollama model
