...
- `consensus_threshold` (str or float, default="unanimous"): Agreement threshold for ensemble mode. Options: `"unanimous"` (100%, default — best accuracy); `"majority"` — STRICT majority (more than half), so 50/50 ties on even-model ensembles resolve to `"0"` (matches sklearn's `VotingClassifier` default and standard ensemble literature); `"two-thirds"` (~67%, uses `>=` semantics); or a custom float between 0 and 1 with `>=` semantics (use `0.5` numerically to keep the older tie-favors-positive behavior). For 2-model ensembles `"majority"` effectively requires both models to agree positive; use 3+ models for non-degenerate majority voting, or pair with `embedding_tiebreaker=True` to resolve true 50/50 ties via centroid similarity.
...
- `json_formatter` (bool or None, default=None): Three-state control for the local JSON-repair model that fixes malformed LLM output before marking responses as failed. The formatter runs only when `extract_json()` produces invalid output — zero cost on the happy path. On first use, the model (~1GB) is downloaded from HuggingFace Hub. Requires `pip install cat-llm[formatter]`. Options: `True` — eagerly load and use the formatter (implicit consent for the ~1.5 GB dependency install if needed); `False` — disabled, malformed rows stay as failures; `None` (default) — auto-prompt on the first malformed row ("Use the formatter for this run? (Y/n)" if deps installed, or "Download deps (~1.5 GB) and use the formatter? (Y/n)" if not). Non-TTY contexts (CI, batch scripts, the desktop app's Streamlit runtime) decline silently and print a one-time suggestion. Auto-enabled when `two_step_classify=True` or any model uses Ollama.
