*! catllm_extract_social -- Discover categories from social media text using catvader
*! Version 1.0.0

program define catllm_extract_social, rclass
    version 16
    ...
    * ----- mark sample -----
    ...
    di as txt "Extracting categories from `nobs' observations (social media domain)..."
    ...
    * ----- call Python -----
    python: _catllm_do_extract_social()

    * ----- display and return -----
    di as txt ""
    di as txt "Extraction complete (social media domain). Categories stored in r()."
    di as txt "Use {bf:return list} to see results."
end

python:
def _catllm_do_extract_social():
    ...
    try:
        result = catvader.extract(**kwargs)
    except Exception as e:
        SFIToolkit.errprintln("{err}catvader.extract() failed: " + str(e))
        return
    ...
    SFIToolkit.displayln("{txt}Discovered categories (social media domain):")
    for i, cat in enumerate(top_cats, 1):
        count_str = ""
