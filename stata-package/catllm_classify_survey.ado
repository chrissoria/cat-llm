*! catllm_classify_survey -- Classify survey text into categories using cat_survey
*! Version 1.0.0

program define catllm_classify_survey, rclass
    version 16
    ...
    * ----- mark sample -----
    ...
    di as txt "Classifying `nobs' observations (survey domain)..."
    ...
    * ----- call Python -----
    python: _catllm_do_classify_survey()
    ...
    * ----- return results -----
    ...
    di as txt ""
    di as txt "Classification complete (survey domain)."
...
python:
def _catllm_do_classify_survey():
    ...
    try:
        result_df = cat_survey.classify(**kwargs)
    except Exception as e:
        SFIToolkit.errprintln("{err}cat_survey.classify() failed: " + str(e))
        return
    ...
    SFIToolkit.displayln("{txt}Python classification complete (survey domain).")
end
